...
export function loadVideo(url) { if (state.videoUrl) URL.revokeObjectURL(state.videoUrl); state.videoUrl = url; state.scale = null; state.scalePoints = []; state.roi = null; state.positions = []; state.tracking = false; state.range = { start: 0, end: 0 }; state.stage = Stage.SCALE; }
